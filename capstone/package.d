...
enum CS_NEXT_VERSION = 3;

// Macro to create combined version which can be compared to
// result of cs_version() API.
uint CS_MAKE_VERSION (ubyte major, ubyte minor) { return ((major<<8)+minor); }
...
alias cs_opt_type = uint;
enum /*cs_opt_type*/ : uint {
...
alias cs_opt_value = uint;
enum /*cs_opt_value*/ : uint {
...
alias cs_op_type = uint;
enum /*cs_op_type*/ : uint {
...
alias cs_ac_type = uint;
enum /*cs_ac_type*/ : uint {
...
alias cs_group_type = uint;
enum /*cs_group_type*/ : uint {
...
//uint CS_INSN_OFFSET(insns, post) (insns[post - 1].address - insns[0].address)
...
*/
public uint cs_version (int* major, int* minor);
...
*/
public bool cs_support (int query);
...
/*
 ...
 for detailed error).
*/
public cs_err cs_open (cs_arch arch, cs_mode mode, csh* handle);
...
/*
 ...
 for detailed error).
*/
public cs_err cs_close (csh* handle);
...
*/
public cs_err cs_errno (csh handle);
...
*/
public const(char)* cs_strerror (cs_err code);
...
*/
public cs_insn* cs_malloc (csh handle);
...
*/
public const(char)* cs_reg_name (csh handle, uint reg_id);
...
*/
public const(char)* cs_insn_name (csh handle, uint insn_id);
...
*/
public const(char)* cs_group_name (csh handle, uint group_id);
...
*/
public bool cs_insn_group (csh handle, const(cs_insn)* insn, uint group_id);
...
*/
public bool cs_reg_read (csh handle, const(cs_insn)* insn, uint reg_id);
...
*/
public bool cs_reg_write (csh handle, const(cs_insn)* insn, uint reg_id);
...
/*
 ...
 or -1 on failure.
*/
public int cs_op_count (csh handle, const(cs_insn)* insn, uint op_type);
...
*/
public int cs_op_index (csh handle, const(cs_insn)* insn, uint op_type, uint position);
...
/*
 ...
 for detailed error).
*/
public cs_err cs_regs_access (csh handle, const(cs_insn)* insn, cs_regs regs_read, ubyte* regs_read_count, cs_regs regs_write, ubyte* regs_write_count);
