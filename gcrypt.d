...
alias gcry_socklen_t = socklen_t;

/* This is required for error code compatibility. */
enum _GCRY_ERR_SOURCE_DEFAULT = GPG_ERR_SOURCE_GCRYPT;
...
enum GCRYPT_VERSION = "1.6.3";

/* The version number of this header.  It may be used to handle minor
   API incompatibilities.  */
enum GCRYPT_VERSION_NUMBER = 0x010603;


/* Wrappers for the libgpg-error library.  */

alias gcry_error_t = gpg_error_t;
alias gcry_err_code_t = gpg_err_code_t;
alias gcry_err_source_t = gpg_err_source_t;

/*GPG_ERR_INLINE*/ gcry_error_t
...
enum GCRY_ERR_SOURCE_DEFAULT = GPG_ERR_SOURCE_USER_1;
...
/*GPG_ERR_INLINE*/ gcry_err_code_t
...
/*GPG_ERR_INLINE*/ gcry_err_source_t
...
/* The version number encoded in the OPTION field of the struct
   gcry_thread_cbs.  */
enum GCRY_THREAD_OPTION_VERSION = 1;
...
alias _GCRY_ATTR_INTERNAL = gcry_thread_cbs;
...
struct gcry_context {}
alias gcry_ctx_t = gcry_context*;

/* The data objects used to hold multi precision integers.  */
struct gcry_mpi {}
alias gcry_mpi_t = gcry_mpi*;
struct gcry_mpi_point {}
alias gcry_mpi_point_t = gcry_mpi_point*;
...
*/
gcry_mpi_t GCRYMPI_CONST_ONE () { return _gcry_mpi_get_const(1); }
gcry_mpi_t GCRYMPI_CONST_TWO () { return _gcry_mpi_get_const(2); }
gcry_mpi_t GCRYMPI_CONST_THREE () { return _gcry_mpi_get_const(3); }
gcry_mpi_t GCRYMPI_CONST_FOUR () { return _gcry_mpi_get_const(4); }
gcry_mpi_t GCRYMPI_CONST_EIGHT () { return _gcry_mpi_get_const(8); }


/* Allocate a new big integer object, initialize it with 0 and
   initially allocate memory for a number of at least NBITS. */
gcry_mpi_t gcry_mpi_new (uint nbits);

/* Same as gcry_mpi_new() but allocate in "secure" memory. */
gcry_mpi_t gcry_mpi_snew (uint nbits);
...
gcry_mpi_t gcry_mpi_set_ui (gcry_mpi_t w, /*unsigned long*/uint u);
...
/* Compare the big integer number U with the unsigned integer V
   ...
   for U < V. */
int gcry_mpi_cmp_ui (const gcry_mpi_t u, /*unsigned long*/uint v);
...
void gcry_mpi_add_ui (gcry_mpi_t w, gcry_mpi_t u, /*unsigned long*/uint v);
...
/* W = U - V.  V is an unsigned integer. */
void gcry_mpi_sub_ui (gcry_mpi_t w, gcry_mpi_t u, /*unsigned long*/uint v );
...
/* W = U * V.  V is an unsigned integer. */
void gcry_mpi_mul_ui (gcry_mpi_t w, gcry_mpi_t u, /*unsigned long*/uint v );

/* W = U * V mod M. */
void gcry_mpi_mulm (gcry_mpi_t w, gcry_mpi_t u, gcry_mpi_t v, gcry_mpi_t m);

/* W = U * (2 ^ CNT). */
void gcry_mpi_mul_2exp (gcry_mpi_t w, gcry_mpi_t u, /*unsigned long*/uint cnt);
...
gcry_mpi_point_t gcry_mpi_point_new (uint nbits);
...
uint gcry_mpi_get_nbits (gcry_mpi_t a);

/* Return true when bit number N (counting from 0) is set in A. */
int      gcry_mpi_test_bit (gcry_mpi_t a, uint n);

/* Set bit number N in A. */
void     gcry_mpi_set_bit (gcry_mpi_t a, uint n);

/* Clear bit number N in A. */
void     gcry_mpi_clear_bit (gcry_mpi_t a, uint n);

/* Set bit number N in A and clear all bits greater than N. */
void     gcry_mpi_set_highbit (gcry_mpi_t a, uint n);

/* Clear bit number N in A and all bits greater than N. */
void     gcry_mpi_clear_highbit (gcry_mpi_t a, uint n);

/* Shift the value of A by N bits to the right and store the result in X. */
void     gcry_mpi_rshift (gcry_mpi_t x, gcry_mpi_t a, uint n);

/* Shift the value of A by N bits to the left and store the result in X. */
void     gcry_mpi_lshift (gcry_mpi_t x, gcry_mpi_t a, uint n);
...
gcry_mpi_t gcry_mpi_set_opaque (gcry_mpi_t a, void *p, uint nbits);
...
gcry_mpi_t gcry_mpi_set_opaque_copy (gcry_mpi_t a,
                                     const(void)* p, uint nbits);

/* Return a pointer to an opaque value stored in A and return its size
   in NBITS.  Note that the returned pointer is still owned by A and
   that the function should never be used for an non-opaque MPI. */
void *gcry_mpi_get_opaque (gcry_mpi_t a, uint *nbits);

/* Set the FLAG for the big integer A.  Currently only the flag
   GCRYMPI_FLAG_SECURE is allowed to convert A into an big intger
   stored in "secure" memory. */
void gcry_mpi_set_flag (gcry_mpi_t a, gcry_mpi_flag flag);

/* Clear FLAG for the big integer A.  Note that this function is
   currently useless as no flags are allowed. */
void gcry_mpi_clear_flag (gcry_mpi_t a, gcry_mpi_flag flag);

/* Return true if the FLAG is set for A. */
int gcry_mpi_get_flag (gcry_mpi_t a, gcry_mpi_flag flag);
...
enum GCRY_CIPHER_AES128      = GCRY_CIPHER_AES;
enum GCRY_CIPHER_RIJNDAEL    = GCRY_CIPHER_AES;
enum GCRY_CIPHER_RIJNDAEL128 = GCRY_CIPHER_AES128;
enum GCRY_CIPHER_RIJNDAEL192 = GCRY_CIPHER_AES192;
enum GCRY_CIPHER_RIJNDAEL256 = GCRY_CIPHER_AES256;

/* The supported encryption modes.  Note that not all of them are
   supported for each algorithm. */
alias gcry_cipher_modes = int;
enum  {
...
enum GCRY_GCM_BLOCK_LEN = (128 / 8);

/* CCM works only with blocks of 128 bits.  */
enum GCRY_CCM_BLOCK_LEN = (128 / 8);

/* Create a handle for algorithm ALGO to be used in MODE.  FLAGS may
   be given as an bitwise OR of the gcry_cipher_flags values. */
gcry_error_t gcry_cipher_open (gcry_cipher_hd_t *handle,
                              int algo, int mode, uint flags);
...
/* Perform the OpenPGP sync operation if this is enabled for the
   cipher handle H. */
auto gcry_cipher_sync (gcry_cipher_hd_t h) { return gcry_cipher_ctl(h, GCRYCTL_CFB_SYNC, null, 0); }

/* Enable or disable CTS in future calls to gcry_encrypt(). CBC mode only. */
auto gcry_cipher_cts (gcry_cipher_hd_t h, int on) { return gcry_cipher_ctl(h, GCRYCTL_SET_CBC_CTS, null, on); }
...
enum {
  GCRY_PK_USAGE_SIGN = 1,   /* Good for signatures. */
  GCRY_PK_USAGE_ENCR = 2,   /* Good for encryption. */
  GCRY_PK_USAGE_CERT = 4,   /* Good to certify other keys. */
  GCRY_PK_USAGE_AUTH = 8,   /* Good for authentication. */
  GCRY_PK_USAGE_UNKN = 128, /* Unknown usage flag. */
}
...
uint gcry_pk_get_nbits (gcry_sexp_t key) /*_GCRY_GCC_ATTR_PURE*/;

/* Return the so called KEYGRIP which is the SHA-1 hash of the public
   key parameters expressed in a way depending on the algorithm.  */
ubyte *gcry_pk_get_keygrip (gcry_sexp_t key, ubyte *array);
...
auto gcry_pk_test_algo (int a) { return gcry_pk_algo_info(a, GCRYCTL_TEST_ALGO, null, null); }
...
/* Create a message digest object for algorithm ALGO.  FLAGS may be
   ...
   gcry_md_enable.  */
gcry_error_t gcry_md_open (gcry_md_hd_t *h, int algo, uint flags);
...
/* Read out the final digest from HD return the digest value for
   algorithm ALGO. */
ubyte *gcry_md_read (gcry_md_hd_t hd, int algo);
...
/* Retrieve the length in bytes of the digest yielded by algorithm
   ALGO. */
uint gcry_md_get_algo_dlen (int algo);
...
alias gcry_mac_hd_t = gcry_mac_handle*;

/* Algorithm IDs for the hash functions we know about. Not all of them
   are implemented. */
alias gcry_mac_algos = int;
enum {
  GCRY_MAC_NONE               = 0,
...
uint gcry_mac_get_algo_keylen (int algo);
...
auto gcry_mac_reset(gcry_mac_hd_t h) { return gcry_mac_ctl(h, GCRYCTL_RESET, null, 0); }

/* Return 0 if the algorithm A is available for use. */
auto gcry_mac_test_algo (int a) { return gcry_mac_algo_info(a, GCRYCTL_TEST_ALGO, null, null); }
...
alias gcry_random_level = int;
alias gcry_random_level_t = gcry_random_level;
enum {
  GCRY_WEAK_RANDOM = 0,
...
auto gcry_fast_random_poll () { return gcry_control(GCRYCTL_FAST_POLL, null); }
...
void gcry_mpi_randomize (gcry_mpi_t w,
                         uint nbits, gcry_random_level level);
...
alias gcry_prime_check_func_t = int function  (void *arg, int mode, gcry_mpi_t candidate);

/* Flags for gcry_prime_generate():  */

/* Allocate prime numbers and factors in secure memory.  */
enum GCRY_PRIME_FLAG_SECRET = (1 << 0);

/* Make sure that at least one prime factor is of size
   `FACTOR_BITS'.  */
enum GCRY_PRIME_FLAG_SPECIAL_FACTOR = (1 << 1);
...
gcry_error_t gcry_prime_check (gcry_mpi_t x, uint flags);
...
alias gcry_handler_log_t = void function (void *, int, const(char)*, va_list);
...
void gcry_set_gettext_handler (const(char)* function (const(char)*) f);
...
auto gcry_fips_mode_active () { return !!gcry_control(GCRYCTL_FIPS_MODE_P, 0); }
