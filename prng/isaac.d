 ...
 */
module iv.prng.isaac /*is aliced*/;
...
public nothrow @trusted @nogc:
  ...
  enum uint max = uint.max; // highest value
  enum bool empty = false; // always `false` (random generators are infinite ranges)

  enum MaxSeedBytes = RANDSIZ*randr[0].sizeof;

  /**
   * Constructs a generator seeded with `rseed`.
   *
   * Params:
   *  rseed = seed vector; up to 256 bytes will be used
   */
  this (const(void)[] rseed) { seed(rseed); }

  /**
   * Constructs a generator seeded with `rseed`.
   *
   * WARNING! this seeding is not crypto-safe!
   *
   * Params:
   *  rseed = seed uint
   */
  this (uint rseed) { seed(rseed); }
   ...
   *
   * Params:
   *  rseedv = seed vector; up to 256 bytes will be used
   */
  void seed (const(void)[] rseedv) {
    enum mix = `
      a ^= b<<11; d += a; b += c;
      b ^= c>>2;  e += b; c += d;
      c ^= d<<8;  f += c; d += e;
      ...
      g ^= h<<8;  b += g; h += a;
      h ^= a>>9;  c += h; a += b;`;
    auto rseed = cast(const(ubyte)[])rseedv;
    ...
    if (rseed.length) {
      ...
      uint xslen = 0;
      uint idx = 0;
      while (xslen < xseed.length && idx < rseed.length) {
        //for (uint i = 0; xslen < xseed.length && i < rseed.length; ++i) {
        uint n = rseed.ptr[idx++];
        if (idx < rseed.length) n |= (cast(uint)(cast(ubyte)rseed.ptr[idx++]))<<8;
        if (idx < rseed.length) n |= (cast(uint)(cast(ubyte)rseed.ptr[idx++]))<<16;
        if (idx < rseed.length) n |= (cast(uint)(cast(ubyte)rseed.ptr[idx++]))<<24;
        xseed.ptr[xslen++] = n;
      }
   ...
   */
  @property uint front () const { pragma(inline, true); return mLastValue; }

  /**
   * Makes a copy of the current prng state.
   */
  @property Isaac save () const {
    import core.stdc.string : memcpy;
    Isaac res;
    memcpy(&res, &this, this.sizeof);
    return res;
  }
   ...
   */
  bool opEquals() (in auto ref Isaac rhs) const pure {
    return
...
unittest {
  ...
  void check (ref Isaac ctx, immutable(uint)[] output) {
  ...
  //
  ctx.seed(cast(immutable(ubyte)[])[42,0,0,0, 154,2,0,0, 205,2,0,0]);
  check(ctx, [
...
unittest {
  import std.stdio;
  auto rng = Isaac(File("/dev/urandom").byChunk(Isaac.MaxSeedBytes).front);
  writeln(rng.front);
}
+/
