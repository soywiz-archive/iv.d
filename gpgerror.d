...
enum GPG_ERR_CODE_MASK = (GPG_ERR_CODE_DIM-1);

/* Bits 17 to 24 are reserved.  */

/* We use the upper 7 bits of gpg_error_t for error sources.  */
enum GPG_ERR_SOURCE_MASK = (GPG_ERR_SOURCE_DIM-1);
enum GPG_ERR_SOURCE_SHIFT = 24;
...
gpg_error_t gpg_err_init () /*_GPG_ERR_CONSTRUCTOR*/;
...
/*GPG_ERR_INLINE*/ gpg_error_t gpg_err_make (gpg_err_source_t source, gpg_err_code_t code) {
...
enum GPG_ERR_SOURCE_DEFAULT = GPG_ERR_SOURCE_UNKNOWN;

/*GPG_ERR_INLINE*/ gpg_error_t gpg_error (gpg_err_code_t code) {
  return gpg_err_make(GPG_ERR_SOURCE_DEFAULT, code);
}


/* Retrieve the error code from an error value.  */
/*GPG_ERR_INLINE*/ gpg_err_code_t gpg_err_code (gpg_error_t err) {
  return cast(gpg_err_code_t)(err&GPG_ERR_CODE_MASK);
}


/* Retrieve the error source from an error value.  */
/*GPG_ERR_INLINE*/ gpg_err_source_t gpg_err_source (gpg_error_t err) {
  return cast(gpg_err_source_t)((err>>GPG_ERR_SOURCE_SHIFT)&GPG_ERR_SOURCE_MASK);
}
...
gpg_err_code_t gpg_err_code_from_syserror ();
...
/*GPG_ERR_INLINE*/ gpg_error_t gpg_err_make_from_errno (gpg_err_source_t source, int err) {
  return gpg_err_make(source, gpg_err_code_from_errno (err));
}


/*GPG_ERR_INLINE*/ gpg_error_t gpg_error_from_errno (int err) {
  return gpg_error(gpg_err_code_from_errno (err));
}

/*GPG_ERR_INLINE*/ gpg_error_t gpg_error_from_syserror () {
...
alias _gpgrt_stream_internal_ptr = void*; //struct
...
alias gpgrt_stream_t = _gpgrt__stream*;
/*
...
alias gpgrt_cookie_io_functions_t = _gpgrt_cookie_io_functions;
/*
#ifdef GPGRT_ENABLE_ES_MACROS
...
alias gpgrt_syshd_types = int;

struct _gpgrt_syshd {
  gpgrt_syshd_types type;
  union {
    int fd;
    int sock;
    int rvid;
...
alias gpgrt_syshd_t = _gpgrt_syshd;
/*
#ifdef GPGRT_ENABLE_ES_MACROS
...
gpgrt_stream_t gpgrt_freopen (scope const(char)* path,
                              scope const(char)* mode,
                              scope gpgrt_stream_t stream);
gpgrt_stream_t gpgrt_fopencookie (scope void *cookie,
                                  scope const(char)* mode,
...
auto gpgrt_stdin () { return _gpgrt_get_std_stream(0); }
auto gpgrt_stdout () { return _gpgrt_get_std_stream(1); }
auto gpgrt_stderr () { return _gpgrt_get_std_stream(2); }
...
auto gpgrt_pending (gpgrt_stream_t stream) { return _gpgrt_pending(stream); }
...
int gpgrt_fseek (gpgrt_stream_t stream, int/*long int*/ offset, int whence);
int gpgrt_fseeko (gpgrt_stream_t stream, gpgrt_off_t offset, int whence);
int/*long int*/ gpgrt_ftell (gpgrt_stream_t stream);
...
int gpgrt_fprintf_unlocked (scope gpgrt_stream_t stream, scope const(char)* format, ...) /*_GPGRT_GCC_A_PRINTF(2,3)*/;

int gpgrt_printf (scope const(char)* format, ...) /*_GPGRT_GCC_A_PRINTF(1,2)*/;
int gpgrt_printf_unlocked (scope const(char)* format, ...) /*_GPGRT_GCC_A_PRINTF(1,2)*/;
...
gpgrt_stream_t gpgrt_tmpfile ();

void gpgrt_opaque_set (scope gpgrt_stream_t stream, scope void *opaque);
...
int gpgrt_asprintf (char **r_buf, scope const(char)* format, ...) /*_GPGRT_GCC_A_PRINTF(2,3)*/;
