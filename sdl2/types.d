...
void SDL_VERSION (ref SDL_version x) @safe pure {
...
uint SDL_DEFINE_PIXELFORMAT(T0, T1, T2, T3, T4) (T0 type, T1 order, T2 layout, T3 bits, T4 bytes)
if (isIntegral!T0 && isIntegral!T1 && isIntegral!T2 && isIntegral!T3 && isIntegral!T4)
{
...
bool SDL_ISPIXELFORMAT_INDEXED(T) (T format) if (isIntegral!T) {
  if (!SDL_ISPIXELFORMAT_FOURCC(format)) {
    immutable auto pixelType = SDL_PIXELTYPE(format);
...
bool SDL_ISPIXELFORMAT_ALPHA(T) (T format) if (isIntegral!T) {
  if (!SDL_ISPIXELFORMAT_FOURCC(format)) {
    immutable auto pixelOrder = SDL_PIXELORDER (format);
...
// SDL_SysWMmsg might have a different size that in SDL
struct SDL_SysWMmsg {
  SDL_version version_;
  SDL_SYSWM_TYPE subsystem;
  // other data i don't care for now
}
...
enum uint SDL_TOUCH_MOUSEID = cast(uint)-1;
