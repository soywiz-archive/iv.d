...
__gshared stbtt_fontinfo ttfont;
//public int stbtt_InitFont(stbtt_fontinfo* info, const(ubyte)* data, int offset);

void ttfLoad () {
  auto fl = VFile(ttffilename);
  if (fl.size < 16 || fl.size > int.max/8) throw new Exception("invalid ttf size");
  import core.stdc.stdlib : malloc;
  ttfontdata = cast(ubyte*)malloc(cast(uint)fl.size);
  fl.rawReadExact(ttfontdata[0..cast(uint)fl.size]);
  if (!stbtt_InitFont(&ttfont, ttfontdata, 0)) throw new Exception("can't load ttf");
}
...
void ttfDrawTextUtf (int x, int y, const(char)[] str, uint fg) {
  ...
  Utf8DecoderFast ud;
  int prevcp = -1;
  immutable float scale = stbtt_ScaleForPixelHeight(&ttfont, 16);
  foreach (char ch; str) {
    if (ud.decode(cast(ubyte)ch)) {
      int dc = (ud.complete ? ud.codepoint : ud.replacement);
      int wdt, hgt, xofs, yofs;
      auto bmp = stbtt_GetCodepointBitmap(&ttfont, scale, scale, dc, &wdt, &hgt, &xofs, &yofs);
      if (bmp !is null) {
        int kadv = 0;
        if (prevcp != -1) {
          kadv = stbtt_GetCodepointKernAdvance(&ttfont, prevcp, dc);
        }
        x += cast(int)(kadv*scale);
        import core.stdc.stdlib : free;
        scope(exit) free(bmp);
        conwriteln("dc=", dc, "; size=(", wdt, "x", hgt, "); xofs=", xofs, "; yofs=", yofs, "; kadv=", kadv*scale);
        foreach (immutable dy; 0..hgt) {
          foreach (immutable dx; 0..wdt) {
            immutable ubyte v = cast(ubyte)(255-bmp[dy*wdt+dx]);
            gxPutPixel(x+dx+xofs, y+dy+yofs, fg|(v<<24));
          }
        }
        x += wdt;
        prevcp = dc;
