...
bool drflac__seek_past_next_set_bit (drflac_bs* bs, uint* pOffsetOut) {
    ...
    if (setBitOffsetPlus1 == 0) {
        if (bs.cache == 1) {
            ...
            for (;;)
            {
                if ((bs.cache & mixin(DRFLAC_CACHE_L1_SELECT!("bs", "setBitOffsetPlus1")))) {
                    break;
                }
...
bool drflac__seek_to_byte (drflac_bs* bs, ulong offsetFromStart) {
    ...
    if (offsetFromStart > 0x7FFFFFFF)
    {
        ...
        if (bytesRemaining > 0) {
            if (!bs.onSeek(bs.pUserData, cast(int)bytesRemaining, drflac_seek_origin_current)) {
                return false;
            }
        }
    }
    ...
    {
        if (!bs.onSeek(bs.pUserData, cast(int)offsetFromStart, drflac_seek_origin_start)) {
            return false;
        }
    }
...
bool drflac__read_utf8_coded_number (drflac_bs* bs, ulong* pNumberOut) {
    ...
    ulong result = cast(ulong)(utf8[0] & (0xFF >> (byteCount + 1)));
...
int drflac__calculate_prediction_64 (uint order, int shift, const(short)* coefficients, int* pDecodedSamples)
{
  ...
  version(DRFLAC_64BIT) {
    ...
    if (order == 8)
    {
        prediction  = coefficients[0] * cast(long)pDecodedSamples[-1];
        prediction += coefficients[1] * cast(long)pDecodedSamples[-2];
        prediction += coefficients[2] * cast(long)pDecodedSamples[-3];
        prediction += coefficients[3] * cast(long)pDecodedSamples[-4];
        prediction += coefficients[4] * cast(long)pDecodedSamples[-5];
        prediction += coefficients[5] * cast(long)pDecodedSamples[-6];
        prediction += coefficients[6] * cast(long)pDecodedSamples[-7];
        prediction += coefficients[7] * cast(long)pDecodedSamples[-8];
    }
    else if (order == 7)
    {
        prediction  = coefficients[0] * cast(long)pDecodedSamples[-1];
    ...
    else if (order == 12)
    {
        ...
        prediction += coefficients[6]  * cast(long)pDecodedSamples[-7];
        prediction += coefficients[7]  * cast(long)pDecodedSamples[-8];
        prediction += coefficients[8]  * cast(long)pDecodedSamples[-9];
        prediction += coefficients[9]  * cast(long)pDecodedSamples[-10];
        prediction += coefficients[10] * cast(long)pDecodedSamples[-11];
        prediction += coefficients[11] * cast(long)pDecodedSamples[-12];
    }
    else if (order == 2)
    {
        prediction  = coefficients[0] * cast(long)pDecodedSamples[-1];
    ...
    else
    {
        ...
        for (int j = 0; j < cast(int)order; ++j) {
            prediction += coefficients[j] * cast(long)pDecodedSamples[-j-1];
        }
    }
  }
...
"        uint decodedRice;\n"~
"\n"~
"        uint setBitOffsetPlus1 = bitOffsetTable["~DRFLAC_CACHE_L1_SELECT_AND_SHIFT!("bs", "4")~"];\n"~
"        if (setBitOffsetPlus1 > 0) {\n"~
...
bool drflac__decode_samples__fixed(drflac_bs* bs, uint blockSize, uint bitsPerSample, ubyte lpcOrder, int* pDecodedSamples)
{
    ...
    if (!drflac__decode_samples_with_residual(bs, bitsPerSample, blockSize, lpcOrder, 0, lpcCoefficientsTable[lpcOrder].ptr, pDecodedSamples)) {
        return false;
    }
...
bool drflac__decode_samples__lpc(drflac_bs* bs, uint blockSize, uint bitsPerSample, ubyte lpcOrder, int* pDecodedSamples)
{
    ...
    for (ubyte i = 0; i < lpcOrder; ++i) {
        if (!drflac__read_int16(bs, lpcPrecision, coefficients.ptr + i)) {
            return false;
        }
    }

    if (!drflac__decode_samples_with_residual(bs, bitsPerSample, blockSize, lpcOrder, lpcShift, coefficients.ptr, pDecodedSamples)) {
        return false;
    }
...
bool drflac__decode_frame(drflac* pFlac)
{
    ...
    if (!drflac__seek_bits(&pFlac.bs, (mixin(DRFLAC_CACHE_L1_BITS_REMAINING!"(&pFlac.bs)") & 7) + 16)) {
        return false;
    }
...
bool drflac__read_streaminfo(drflac_read_proc onRead, void* pUserData, drflac_streaminfo* pStreamInfo)
{
    ...
    // MD5
    ubyte[16] md5;
    if (onRead(pUserData, md5.ptr, md5.sizeof) != md5.sizeof) {
        return false;
    }
...
//#ifndef DR_FLAC_NO_OGG
bool drflac_ogg__is_capture_pattern(ubyte* pattern/*[4]*/)
{
...
{
    ubyte[4] id;
    if (onRead(pUserData, id.ptr, 4) != 4) {
        return false;
    }
...
bool drflac_oggbs__seek_physical(drflac_oggbs* oggbs, ulong offset, drflac_seek_origin origin)
{
    if (origin == drflac_seek_origin_start)
    {
        if (offset <= 0x7FFFFFFF) {
            if (!oggbs.onSeek(oggbs.pUserData, cast(int)offset, drflac_seek_origin_start)) {
                return false;
            }
    ...
    else
    {
        ...
        if (!oggbs.onSeek(oggbs.pUserData, cast(int)offset, drflac_seek_origin_current)) {    // <-- Safe cast thanks to the loop above.
            return false;
        }
        oggbs.currentBytePos += offset;
    ...
    for (;;)
    {
        ...
        if (pageBodySize == 51)   // 51 = the lacing value of the FLAC header packet.
        {
            ...
            if (packetType == 0x7F)
            {
                ...
                ubyte[4] sig;
                if (onRead(pUserData, sig.ptr, 4) != 4) {
                    return false;
                }
                ...
                if (sig[0] == 'F' && sig[1] == 'L' && sig[2] == 'A' && sig[3] == 'C')
                {
                    ...
                    if (onRead(pUserData, mappingVersion.ptr, 2) != 2) {
                        return false;
                    }
...
{
    if (pInit is null || onRead is null || onSeek is null) {
        return false;
    }
...
void drflac__init_from_info(drflac* pFlac, drflac_init_info* pInit)
{
    ...
    pFlac.bs.nextL2Line    = (pFlac.bs.cacheL2).sizeof / (pFlac.bs.cacheL2[0]).sizeof; // <-- Initialize to this to force a client-side data retrieval right from the start.
...
//#ifndef DR_FLAC_NO_OGG
    ...
    if (init.container == drflac_container_ogg) {
        allocationSize += (drflac_oggbs).sizeof;
    }
//#endif
...
ulong drflac__read_s32__misaligned(drflac* pFlac, ulong samplesToRead, int* bufferOut)
{
    ...
    while (samplesToRead > 0)
    {
        ...
        switch (pFlac.currentFrame.header.channelAssignment)
        {
            ...
            {
                if (channelIndex == 0) {
                    decodedSample = pFlac.currentFrame.subframes[channelIndex].pDecodedSamples[cast(uint)nextSampleInFrame];
                } else {
                    int side = pFlac.currentFrame.subframes[channelIndex + 0].pDecodedSamples[cast(uint)nextSampleInFrame];
                    int left = pFlac.currentFrame.subframes[channelIndex - 1].pDecodedSamples[cast(uint)nextSampleInFrame];
            ...
            case DRFLAC_CHANNEL_ASSIGNMENT_RIGHT_SIDE:
            {
                if (channelIndex == 0) {
                    int side  = pFlac.currentFrame.subframes[channelIndex + 0].pDecodedSamples[cast(uint)nextSampleInFrame];
                    int right = pFlac.currentFrame.subframes[channelIndex + 1].pDecodedSamples[cast(uint)nextSampleInFrame];
            ...
            case DRFLAC_CHANNEL_ASSIGNMENT_MID_SIDE:
            {
                ...
                int side;
                if (channelIndex == 0) {
                    mid  = pFlac.currentFrame.subframes[channelIndex + 0].pDecodedSamples[cast(uint)nextSampleInFrame];
                    side = pFlac.currentFrame.subframes[channelIndex + 1].pDecodedSamples[cast(uint)nextSampleInFrame];

                    mid = ((cast(uint)mid) << 1) | (side & 0x01);
                    decodedSample = (mid + side) >> 1;
                } else {
                    mid  = pFlac.currentFrame.subframes[channelIndex - 1].pDecodedSamples[cast(uint)nextSampleInFrame];
                    side = pFlac.currentFrame.subframes[channelIndex + 0].pDecodedSamples[cast(uint)nextSampleInFrame];
...
public ulong drflac_read_s32(drflac* pFlac, ulong samplesToRead, int* bufferOut)
{
    ...
    if (pFlac is null || samplesToRead == 0) {
        return 0;
    }
                    ...
                    else
                    {
                        ...
                        for (/*ulong*/uint i = 0; i < alignedSampleCountPerChannel; ++i) {
                            for (uint j = 0; j < channelCount; ++j) {
                                bufferOut[(i*channelCount)+j] = (pFlac.currentFrame.subframes[j].pDecodedSamples[cast(uint)firstAlignedSampleInFrame + i]) << (unusedBitsPerSample + pFlac.currentFrame.subframes[j].wastedBitsPerSample);
                            }
                        }
                    }
                } break;
...
public const(char)* drflac_next_vorbis_comment(drflac_vorbis_comment_iterator* pIter, uint* pCommentLengthOut)
{
    ...
    if (pIter is null || pIter.countRemaining == 0 || pIter.pRunningData is null) {
        return null;
    }
