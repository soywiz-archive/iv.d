...
void testShift(uint S) (const(ubyte)[] key, const(ubyte)[] output) {
  auto ctx = ARC4Engine!S(key);
  ubyte[16] res;
  ctx.process(res[], res[]); // use slicing, 'cause static arrays don't have popFront
  assert(res == output);
  ++count;
}


void test (const(ubyte)[] key, const(ubyte)[16][18] output) {
  // aaaah, let the fun begin!
  string xgen (immutable(uint[]) skips) {
    string res = "";
    foreach (immutable i, immutable n; skips) {
      res ~= `testShift!`~to!string(n)~`(key, output[`~to!string(i)~`]);`;
    }
    return res;
  }

  mixin(xgen([0,16,240,256,496,512,752,768,1008,1024,1520,1536,2032,2048,3056,3072,4080,4096]));
}


void main () {

  writeln("testing ARC4...");
  ...
  test([0x01,0x02,0x03,0x04,0x05,0x06,0x07], [
  ...
  test([0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a], [
  ...
  test([0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,0x10], [
  ...
  test([0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18], [
  ...
  test([0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x1a,0x1b,0x1c,0x1d,0x1e,0x1f,0x20], [
  ...
  test(cast(const(ubyte[]))x"833222772a", [
  ...
  test(cast(const(ubyte[]))x"1910833222772a", [
  ...
  test(cast(const(ubyte[]))x"641910833222772a", [
  ...
  test(cast(const(ubyte[]))x"8b37641910833222772a", [
  ...
  test(cast(const(ubyte[]))x"ebb46227c6cc8b37641910833222772a", [
  ...
  test(cast(const(ubyte[]))x"c109163908ebe51debb46227c6cc8b37641910833222772a", [
  ...
  test(cast(const(ubyte[]))x"1ada31d5cf688221c109163908ebe51debb46227c6cc8b37641910833222772a", [
