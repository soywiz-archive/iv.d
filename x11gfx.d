...
private {
  void blit2xImpl(string op, bool scanlines=true) (Image img) {
    enum ShineMixin = q{
      if (blitShine && c0) {
        /*
        immutable ubyte xc0 = clampToByte(cast(int)(c0&0xff ? cast(int)(c0&0xff)+blitShine : 0));
        immutable ubyte xc1 = clampToByte(cast(int)((c0>>8)&0xff ? cast(int)((c0>>8)&0xff)+blitShine : 0));
        immutable ubyte xc2 = clampToByte(cast(int)((c0>>16)&0xff ? cast(int)((c0>>16)&0xff)+blitShine : 0));
        */
        immutable ubyte xc0 = clampToByte(cast(int)(cast(int)(c0&0xff)+blitShine));
        immutable ubyte xc1 = clampToByte(cast(int)(cast(int)((c0>>8)&0xff)+blitShine));
        immutable ubyte xc2 = clampToByte(cast(int)(cast(int)((c0>>16)&0xff)+blitShine));
        c0 = xc0|(xc1<<8)|(xc2<<16);
      }
    };
  ...
  alias blit2xTV = blit2xImpl!"uint c0 = (cast(immutable(uint)*)s)[0];";
  alias blit2xTVBW = blit2xImpl!"immutable ubyte i = cast(ubyte)((s[0]*28+s[1]*151+s[2]*77)/256); uint c0 = (i<<16)|(i<<8)|i;";
  alias blit2xTVGreen = blit2xImpl!"immutable ubyte i = cast(ubyte)((s[0]*28+s[1]*151+s[2]*77)/256); uint c0 = i<<8;";
  alias blit2xTVRed = blit2xImpl!"immutable ubyte i = cast(ubyte)((s[0]*28+s[1]*151+s[2]*77)/256); uint c0 = i<<16;";
}
  ...
  */
  if (blit2x) {
    ...
    switch (blitType) {
      ...
      case BlitType.Red: blit2xTVRed(img); break;
